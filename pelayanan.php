...
if (isset($_POST['btnSimpan'])) {
    ...
    $check_duplicate = mysqli_query($koneksi, "SELECT COUNT(*) FROM pelayanan WHERE id_pelayanan = '$id_pelayanan'");
...
if (isset($_GET['hal']) && $_GET['hal'] == 'hapus') {
    ...
    if (isset($_GET['no_pel']) && !empty($_GET['no_pel'])) {
        $no_pelayanan_to_delete = $_GET['no_pel'];

        // Ambil id_buka_rekening dan no_registrasi dari data yang akan dihapus
        $query_get_data = mysqli_query($koneksi, "SELECT id_pelayanan, no_pelayanan FROM pelayanan WHERE no_pelayanan = '$no_pelayanan_to_delete'");
        $data_to_delete = mysqli_fetch_assoc($query_get_data);

        // Lakukan query penghapusan
        $hapus = mysqli_query($koneksi, "DELETE FROM pelayanan WHERE no_pelayanan = '$no_pelayanan_to_delete'");

        if ($hapus) {
            // Perbarui nilai id_buka_rekening dan no_registrasi untuk data dengan id_buka_rekening lebih besar
            mysqli_query($koneksi, "UPDATE pelayanan SET 
                                    id_pelayanan = id_pelayanan - 1,
                                    no_pelayanan = no_pelayanan - 1
                                    WHERE id_pelayanan > '{$data_to_delete['id_pelayanan']}'");
...
if (isset($_GET['hal'])) {
    ...
    if ($_GET['hal'] == "edit") {
        ...
        $tampil = mysqli_query($koneksi, "SELECT * FROM pelayanan WHERE no_pelayanan = '$_GET[no_pel]'");
        $data = mysqli_fetch_array($tampil);

        if ($data) {
            // data ditemukan
            $vIdpelayanan = $data['id_pelayanan'];
            $vNama = $data['nama'];
            $vtglPelayanan = $data['tanggal_pelayanan'];
            $vJamoperasional = $data['jam_oprasional'];
            $vDeskripsi = $data['deskripsi'];
            $vIdnasabah = $data['id_nasabah'];
            $vIdcs = $data['id_cs'];
            $vJenispelayanan = $data['id_jenis'];
        }
...
{
    $query_get_max_id = mysqli_query($koneksi, "SELECT MAX(id_pelayanan) as max_id FROM pelayanan");
    $result = mysqli_fetch_assoc($query_get_max_id);
    $max_id = $result['max_id'];

    if ($max_id === null) {
        // Jika belum ada data di database,
        return 'SRC44001'; // ID awal jika database kosong
    } else {
            ...
            <div class="grid grid-cols-2 gap-4 sm:grid-cols-2 mb-2 mt-2">
                            ...
                            <form method="POST" action="pelayanan.php">
                                <div class="grid grid-cols-2 gap-4 sm:grid-cols-2 mb-2 ">
                                    <div class="mb-2">
                                        ...
                                        <input type="text" id="newIdInput"
                                            ...
                                            placeholder="Masukkan Id Pelayanan" name="id_pelayanan"
                                            value="<?= $vIdpelayanan ?>" value="<?= $new_id ?>" readonly
                                            id="newIdInput">
                                    </div>
                                    ...
                                    <div class="mb-2">
                                        ...
                                        <input type="text" id="iptNama"
                                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                                            placeholder="Masukan Nama" name="nama" value="<?= $vNama ?>">
                                    </div>
                                </div>
                                ...
                                <div class="grid grid-cols-1 gap-4 sm:grid-cols-3 mb-2">
                                    <div class="mb-2">
                                        ...
                                        <input type="date" id="tglPelayanan"
                                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                                            placeholder="Masukan Tanggal" name="tglPelayanan"
                                            value="<?= $vtglPelayanan ?>">
                                    </div>
                                    ...
                                    <div class="mb-2">
                                        ...
                                        <input type="time" id="iptNama"
                                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                                            placeholder="Masukkan Jam Operasional" name="jam_operasional"
                                            value="<?= $vJamoperasional ?>">
                                    </div>
                                    ...
                                    <div class="mb-2">
                                        ...
                                        <input type="text" id="iptDeskripsi"
                                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                                ...
                                <div class="grid grid-cols-1 gap-4 sm:grid-cols-1 mt-0 mb-1">
                                    <div>
                                        <label for="default"
                                            class="block mb-1 ml-2 text-sm font-medium text-gray-900 dark:text-white">
                                            Jenis Pelayanan
                                        ...
                                        <select id="default"
                                            ...
                                            $jenis_pelayanan_options = [
                                                "1" => "Aktivasi M-Banking",
                                                "2" => "Pembukaan Rekening",
                                                "3" => "Penutupan Rekening",
                                                "4" => "Pemblokiran Rekening",
                                                "5" => "Administrasi",
                                                "6" => "Solve Problem Nasabah",


                                            ];

                                            foreach ($jenis_pelayanan_options as $value => $label) {
                                                // Tentukan apakah opsi saat ini harus dipilih
                                                $selected = ($vJenispelayanan == $value) ? 'selected' : '';
                ...
                <div class="mt-10">
                    ...
                    <p class="mb-6 text-lg font-normal text-gray-500 lg:text-xl sm:px-3 xl:px-3 dark:text-gray-400">
                        Harap masukan data dengan teliti dan benar !</p>
                </div>
            ...
            <table id="dataTable" class="min-w-full divide-y-2 divide-white-200 bg-white text-sm">
                <thead class="ltr:text-center rtl:text-center sticky-header text-white">
                    <tr class="text-center">
                        <th style="width: 10%; padding: 0.75rem;">Id Pelayanan</th>
                        <th style="width: 15%; padding: 0.75rem;">Nama</th>
                        <th style="width: 10%; padding: 0.75rem;">Tanggal Pelayanan</th>
                        <th style="width: 10%; padding: 0.75rem;">Jam Operasional</th>
                        <th style="width: 10%; padding: 0.75rem;">Deskripsi</th>
                        <th style="width: 10%; padding: 0.75rem;">Id Nasabah</th>
                        <th style="width: 10%; padding: 0.75rem;">Id Cs</th>
                        <th style="width: 10%; padding: 0.75rem;">Jenis Pelayanan</th>
                ...
                <tbody class="divide-y divide-gray-200">
                    ...
                    $tampil = mysqli_query($koneksi, "SELECT * FROM pelayanan ORDER BY id_pelayanan DESC");
                        ...
                        <tr class=" text-gray-700">
                            ...
                            <td>
                                ...
                                <button class="mt-3 mb-2">
                                    <a href="pelayanan.php?hal=hapus&no_pel=<?= $data['no_pelayanan'] ?>"
                                ...
                                <button class="mt-3 mb-2">
                                    <a href="pelayanan.php?hal=edit&no_pel=<?= $data['no_pelayanan'] ?>"
        ...
        $(document).ready(function () {
            ...
            function refreshTable(searchValue) {
                var filteredData = originalData.filter(function (data) {
                    return (
                        data.id_pelayanan.includes(searchValue) ||
                        data.nama.toLowerCase().includes(searchValue) ||
                        data.id_nasabah.toLowerCase().includes(searchValue)
                    );
                });
                ...
                $.each(filteredData, function (index, data) {
                    $("#dataTable tbody").append(
                        "<tr>" +
                        "<td class='text-center'>" + data.no_pelayanan + "</td>" +
                        "<td class='text-center'>" + data.id_pelayanan + "</td>" +
                        "<td class='text-center'>" + data.nama + "</td>" +
                        "<td class='text-center'>" + data.tanggal_pelayanan + "</td>" +
                        "<td class='text-center'>" + data.jam_oprasional + "</td>" +
                        "<td class='text-center'>" + data.deskripsi + "</td>" +
                        "<td class='text-center'>" + data.id_nasabah + "</td>" +
                        "<td class='text-center'>" + data.cs_id_cs + "</td>" +
                        "<td class='text-center'>" + data.jenis_pelayanan_id_jenis + "</td>" +
                        "</tr>"
                    );
                });
            }
